...
define( 'VOSIO_ROOT', get_template_directory() . '/' );
...
add_action( 'after_setup_theme', 'vosio_wp_load', 5 );

function vosio_wp_load() {

	defined( 'VOSIO_URL' ) or define( 'VOSIO_URL', get_template_directory_uri() . '/' );
	define(  'VOSIO_KEY','!@#vosio');
	define(  'VOSIO_URI', get_template_directory_uri() . '/');

	if ( ! defined( 'VOSIO_NONCE' ) ) {
		define( 'VOSIO_NONCE', 'vosio_wp_theme' );
	}

	( new \VOSIO\Includes\Classes\Base )->loadDefaults();
	( new \VOSIO\Includes\Classes\Ajax )->actions();

}
add_action( 'init', 'vosio_bunch_theme_init');
function vosio_bunch_theme_init()
{
