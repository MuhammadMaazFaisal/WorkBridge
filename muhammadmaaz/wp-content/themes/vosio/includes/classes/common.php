...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => vosio_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => vosio_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => vosio_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => vosio_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => vosio_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => vosio_meta( 'banner_elementor_template', $page_id ),
				'banner'            => vosio_set( vosio_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => vosio_meta( 'banner_banner_title', $page_id ),
				'text'             => vosio_meta( 'banner_banner_text', $page_id ),
				'enable_banner'     => vosio_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : vosio_the_title( $template ),
				'text'         => $options->get( $text ) ? $options->get( $text ) : vosio_the_title( $template ),
				'banner'        => vosio_set( $bg, 'url' ),
